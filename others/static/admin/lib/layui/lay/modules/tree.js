            ...
            var hasChild = item.children && item.children.length > 0
                ...
                entryDiv = $(['<div data-id="' + item.id + '" class="layui-tree-set' + (item.spread ? " layui-tree-spread" : "") + (item.checked ? " layui-tree-checkedFirst" : "") + '">'
                    ...
                    , function () {
                        ...
                        // 如果当前的rank大于等于指定的limitNodeAddLevel 则不显示加号图标
                        if (options.limitNodeAddLevel > 0 && item.rank >= options.limitNodeAddLevel) {
                            editIcon = {
                                update: '<i class="layui-icon layui-icon-edit" data-type="update"></i>'
                                , del: '<i class="layui-icon layui-icon-delete" data-type="del"></i>'
                            }
                        }
                        // 如果当前的ID 大于 30000 则不显“+”图标
                        if (item.id != null && item.id >= 30000) {
                            editIcon = {
                                update: '<i class="layui-icon layui-icon-edit" data-type="update"></i>'
                                , del: '<i class="layui-icon layui-icon-delete" data-type="del"></i>'
                            }
                        }
                        // 如果当前的rank为1 则不显示 “删除” 图标
                        if (item.id != null && item.rank == 1) {
                            editIcon = {
                                add: '<i class="layui-icon layui-icon-add-1"  data-type="add"></i>'
                                , update: '<i class="layui-icon layui-icon-edit" data-type="update"></i>'
                            }
                        ...
                        // if (options.limitNodeDelLevel > 0 && item.rank <= options.limitNodeDelLevel) {
                        //     editIcon = {
                        //         add: '<i class="layui-icon layui-icon-add-1"  data-type="add"></i>'
                        //         , update: '<i class="layui-icon layui-icon-edit" data-type="update"></i>'
                        //     }
    ...
    Class.prototype.operate = function (elem, item) {
        ...
        entry.children('.layui-tree-btnGroup').on('click', '.layui-icon', function (e) {
            ...
            if (type == 'add') {
                ...
                // 判断当前结点是否有子节点 有则返回false 没有则返回true
                console.log(!elem.children('.' + ELEM_PACK)[0])
                if (!elem.children('.' + ELEM_PACK)[0] == false) {
                    layer.msg("当前结点含有子节点，不能删除")
                    return;
                }
